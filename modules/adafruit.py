...
def checkPage(id):
    ...
    r = requests.get(f"https://www.adafruit.com/api/product/{id}")
    ...
    if r.status_code == 200:
        ...
        if data['product_stock'] != "0":
            r_details['stock'] = getStock(data['product_stock']) # get stock from page
            # return product data to send out restock alert
            r_details['instock'] = True
            r_details['method'] = 'adafruit api'
    return r_details

# get stock from response when stock isn't zero
def getStock(stock):
